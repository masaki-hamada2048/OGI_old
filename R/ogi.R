...
#' @param nu Numeric vector of subjective importance. It determines the importance of each of the variates.
#' @param tol Numeric number of tolerance. If the minimum eigenvalue of \code{S} is less than \code{tol},
#'     \code{S} is considered not to be positive definite.
#' @param force Logical: If force=FALSE, \code{S} should be strictly positive definite. Default: FALSE.
#' @return Numeric vector of diagonal elements of \eqn{D}, which appears in the bi-unit canonical form \eqn{DSD} of \eqn{S}.
#' @examples
#' S = matrix(0, 5, 5)
#' S[1,1] = 1
...
#'
#' \code{cov2biu(S)} returns the bi-unit canonical form of \eqn{S}.
#'
#' @param S Covariance matrix, especially it is positive semi-definite.
#' @param detail Logical: if \code{detail=TRUE}, it returns the list of the bi-unit form and the weight vectors.
#'               Default: FALSE.
...
#' B
#' @export
cov2biu = function(S, detail=FALSE){ # covariance matrix to bi-unit matrix
  w = cov2weight(S)
  B = diag(w) %*% S %*% diag(w)
...
#'
#' \code{ogi(X)} returns the objective general index (OGI) of the covariance matrix \code{S} of \code{X}.
#'
#' Consider a data matrix of \eqn{n} individuals with \eqn{p} variates. The objective general index (OGI)
#' is a general index that combines the \eqn{p} variates into a univariate index in order to rank the
#' \eqn{n} individuals. The OGI is always positively correlated with each of the variates.
#' For more details, see the references.
#'
#' @param X Numeric or ordered matrix.
#' @param se Logical: if \code{se=TRUE}, it additionally computes \code{w.se} and \code{v.se} by bootstrap.
#'     Default: FALSE.
#' @param force Logical: If force=FALSE, \code{S} should be strictly positive definite. Default: FALSE.
#' @param se.loop Iteration number in bootstrap for computation of standard error.
#' @param nu Numeric vector of subjective importance. It determines the importance of each column of \code{X}.
#' @param center Logical: if center=TRUE, \code{ogi(X)$Z} is centered. Default:TRUE.
#' @param mar Logical: if mar=TRUE, \code{X} is supposed to be the ordered categorical data. Default:FALSE.

#' @return
#'   \item{value}{The objective general index (OGI).}
#'   \item{X}{The input matrix \code{X}.}
#'   \item{scaled}{The product of \code{Z \%*\% diag(weight)},
#'     where \code{Z} and \code{weight} are as follows.}
#'   \item{Z}{Numerical matrix converted from X. If center = TRUE, it is centered.}
#'   \item{weight}{The output of \code{\link{cov2weight}(S, nu=nu, force=force)}. }
#'   \item{rel.weight}{The product of \code{weight * sqrt(diag(S))},
...
#'   \item{w.se}{If requested, \code{w.se} is numeric vector of the standard error of
#'     \code{weight}, where \code{S} is the covariance matrix of \code{X}.
#'     It is calculated by bootstrap.}
#'   \item{v.se}{If requested, \code{v.se} is numeric vector of the standard error of
#'     \code{ogi(S)$value}, where \code{S} is the covariance matrix of \code{X}.
#'     It is calculated by bootstrap.}
#' @references  Sei, T. (2016). An objective general index for multivariate ordered data,
